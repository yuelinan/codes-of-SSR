...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for step,batch in enumerate(tqdm(train_shortcut_dataloader)):
            ...
            for p in model.vencoder.parameters():
                p.requires_grad = True
            #### BoostRNP baseline
            
            
            output,log_likelihood= model(input_ids,attention_mask,tag_ids,shortcut_ids)
            loss = criterion(output,label_ids) + log_likelihood + 0.1*model.shortcut_loss + 0.1*criterion(model.shortcut_out,label_ids)
        ...
        for step,batch in enumerate(tqdm(train_dataloader)):
            ...
            loss = criterion(output,label_ids) + args.alpha*model.infor_loss + args.beta*model.regular + 0.1*model.comp_loss 
            loss = loss / args.gradient_accumulation_steps
            loss.backward()
...
def main_with_da():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for step,batch in enumerate(tqdm(train_shortcut_dataloader)):
            ...
            output,log_likelihood= model(input_ids,attention_mask,tag_ids,shortcut_ids)
            loss = criterion(output,label_ids) + log_likelihood + 0.1*model.shortcut_loss + criterion(model.shortcut_out,label_ids)
